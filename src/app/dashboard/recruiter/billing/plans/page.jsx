...
const RECRUITER_PLANS = [
  ...
  },
  {
    ...
    yearlyPrice: "$449.90",
    period: "/month",
    description: "Scale your hiring pipeline faster",
    features: [
  ...
  },
  {
    ...
    icon: Zap,
    monthlyPrice: "$149.99",
    yearlyPrice: "$1,349.90",
    period: "/month",
    description: "Enterprise-grade recruitment at scale",
    features: [
...
function getButtonState(plan, activePlan, billingCycle) {
  ...
  const activeLevel = RECRUITER_PLANS.find((p) => p.id === activePlan.planId)?.level ?? 0;
...
function PlanCard({ plan, billingCycle, activePlan }) {
  ...
  const isFree = plan.id === "recruiter_free";
...
export default function RecruiterBillingPlansPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Back Button */}
      <button
        onClick={() => router.push("/dashboard/recruiter/billing")}
